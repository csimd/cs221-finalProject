...
def unpack_feature_data(cur_song, feature_data, train_keys=None, isTest=False):
	...
	for k, v in feature_data.items():
		...
		artist_key = cur_song['artists'][0]['name']
		if not isTest or (isTest and year_key in train_keys):
			feature_dict[year_key] = 1
		if not isTest or (isTest and artist_key in train_keys):
			feature_dict[artist_key] = 1
	return feature_dict
...
def load_lyrics(allSongs, sp_data_loader, isTest=False, trainSongs=[]):
	...
	for i, ts in enumerate(allSongs):
		...
		if songName and songArtist and songLyrics:
...
#Loads the intersection of the training data from pickle file. Returns a list.
def load_train_data(testing=False):
	'''Keeps track of previously added songs where the artist is the key and the
	val is a list of the songs by the artist that we have'''
	added_songs = {}
	song_set = []
		...
		while len(song_set) <= 1000:
			try:
				...
				for a in dumped:
					...
					if cur_artist not in added_songs:
						added_songs[cur_artist] = [cur_name]
						song_set.append(a)
					elif cur_name not in added_songs[cur_artist]:
						added_songs[cur_artist].append(cur_name)
						song_set.append(a)

	print('TOTAL NUM OF SONGS FOUND: {}'.format(len(song_set)))
...
def main():
	...
	# 	with open("train_song_data_" + 'final' + ".pkl", 'ab') as pkl:
	# 		pickle.dump(trainSongData, pkl)

	# UNPICKLE CODE
	trainSongData = None
	# with open('train_song_data_1543840506.251837.pkl', 'rb') as unpkl:
	# with open('train_song_data_final.pkl', 'rb') as unpkl:
	...
	# testSongs = sp_test.load_song_data(num_songs=2, num_iters=50, offset_max=300)
	# testSongData = load_lyrics(testSongs, sp_test, isTest=True, trainSongs=[t[0] for t in trainSongData])
	#testSongData = []
	# with open('test_sample.pkl', 'ab') as pkl:
	# 	testSongData = pickle.dump(testSongData, pkl)
	#with open('test_sample.pkl', 'rb') as pkl:
	testSongData = None
	with open('test_sample.pkl', 'rb') as unpkl:
		testSongData = pickle.load(unpkl)
		#while True:
			#try:
			#	rax = pickle.load(pkl)
			#except EOFError:break
	...
	print('testData read')
	print('{} FILES IN TEST DATASET'.format(len(testSongData)))
	# testD = load_train_data(testing=True)
	...
	for ___ in range(1):
		...
		# for curr_num_centroids in range(6, 20, 2):
			...
			for trial_count in range(1):
				...
				# with open("weights_per_model.txt", 'wb') as weight_file:
				# 	for i, m in enumerate(models):
				# 		weight_file.write("Model {}: \n ")
		...
		print('topk: \n {}'.format(topk))
		print('lowest loss: {}'.format(min(topk, key = lambda x:x[1] )))
	freq_tbl = {}
	# for opt in topk:
	# 	if opt in topk: topk[opt] += 1
	# 	else: topk[opt] = 1
	# print('consistently lowest: {}'.format(max(topk, key = topk.get() )))
	#Clears lyrics
